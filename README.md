...
- Transcription runs locally with `whisper.cpp` and the faster multilingual Whisper Small Q5 model.
...
The offline installer contains the approximately 190 MB language model. Processing speed depends on the computer's CPU. The video and extracted audio remain on the computer.

## Download

Download `Bangla_Subtitle_Studio_Offline_Fast_Setup_V2.1.exe` from the `v2.1.0` GitHub Release and install it normally. The existing V1 or V2 installation can be upgraded because the installer keeps the same application identity.
...
- verified `ggml-small-q5_1.bin` multilingual model
